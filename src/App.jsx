...
class ErrorBoundary extends Component {
  ...
  render() {
    if (this.state.error) return (
      <div style={{ background:"#080e1a", color:"#fff", padding:32, fontFamily:"monospace", fontSize:13, height:"100vh", overflowY:"auto" }}>
        <div style={{ color:"#00E87A", marginBottom:16, fontSize:16, fontWeight:700 }}>WorldXI — Runtime Error</div>
        <div style={{ color:"#FF4757", marginBottom:12 }}>{this.state.error.message}</div>
        <pre style={{ color:"rgba(255,255,255,0.5)", fontSize:11, whiteSpace:"pre-wrap" }}>{this.state.error.stack}</pre>
      </div>
    );
...
function SetUsernameScreen({ wallet, onDone }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Register onchain — writes to X Layer
