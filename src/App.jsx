...
function VideoSplash({ onDone }) {
  ...
  useEffect(() => {
    // Fallback: if video doesn't load in 10s, skip anyway
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ width:120, height:2, background:"rgba(255,255,255,0.15)", borderRadius:99, overflow:"hidden" }}>
          <div style={{
            height:"100%", background:"var(--accent)",
            borderRadius:99, animation:"splashProgress 8s linear forwards",
          }} />
        </div>
      </div>
...
export default function WorldXIApp() {
  const [phase, setPhase] = useState("video"); // video | connect | username | app
  ...
  return (
    <>
      ...
      {phase === "video" && <VideoSplash onDone={() => setPhase("connect")} />}
